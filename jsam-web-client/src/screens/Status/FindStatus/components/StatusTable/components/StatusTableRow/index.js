...
import { updateStatusOnServerById, deleteStatusOnServerById } from 'Services/status/actions';
...
class StatusTableRow extends React.Component {
  ...
  handleConfirmEdit = async () => {
    ...
    const { _id } = input;
    const statusUpdateData = { ...input };

    this.setState(() => ({ editLoading: true }));
    try {
      const { dispatchUpdateStatusOnServerById } = this.props;
      await dispatchUpdateStatusOnServerById(_id, statusUpdateData);
  ...
  handleDelete = async () => {
    try {
      ...
      const { item } = this.props;
      const { dispatchDeleteStatusOnServerById } = this.props;
      await dispatchDeleteStatusOnServerById(item._id);
  ...
  render() {
    ...
    if (editModeOn) {
      ...
      if (editLoading) {
        editButtonTooltipTitle = 'Updating status...';
      } else if (editPostLoading) {
        if (editSuccess) {
          editButtonTooltipTitle = 'Successfully updated status';
        } else {
          editButtonTooltipTitle = 'Updating status failed';
        }
      }
    }
    ...
    if (deleteLoading) {
      deleteButtonTooltipTitle = 'Deleting status...';
    } else if (deletePostLoading) {
      if (deleteSuccess) {
        deleteButtonTooltipTitle = 'Successfully deleted status';
      } else {
        deleteButtonTooltipTitle = 'Deleting status failed';
      }
    }
...
StatusTableRow.propTypes = {
  ...
  dispatchUpdateStatusOnServerById: PropTypes.func.isRequired
};

const mapDispatchToProps = dispatch => ({
  dispatchUpdateStatusOnServerById: (_id, statusUpdateData) =>
    dispatch(updateStatusOnServerById(_id, statusUpdateData)),
  dispatchDeleteStatusOnServerById: _id => dispatch(deleteStatusOnServerById(_id))
});

export default connect(
  undefined,
  mapDispatchToProps
)(withStyles(styles)(StatusTableRow));
