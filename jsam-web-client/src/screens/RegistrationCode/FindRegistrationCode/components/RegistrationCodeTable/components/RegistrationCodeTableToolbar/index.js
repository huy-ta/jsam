...
class RegistrationCodeTableToolbar extends React.Component {
  state = {
  ...
  render() {
    ...
    let bulkDeleteTooltipTitle = 'Delete selected registrationCodes';
    if (bulkDeleteLoading) {
      bulkDeleteTooltipTitle = 'Deleting registrationCodes...';
    } else if (bulkDeletePostLoading) {
      if (bulkDeleteSuccess) {
        bulkDeleteTooltipTitle = 'Successfully deleted registrationCodes';
      } else {
        bulkDeleteTooltipTitle = "Couldn't delete registrationCodes";
      }
    }
        ...
        <div className={classes.title}>
          {numSelected > 0 ? (
            <Typography color="inherit" variant="subtitle1">
              {numSelected} registrationCode(s) selected
            </Typography>
          ) : (
...
RegistrationCodeTableToolbar.propTypes = {
  classes: PropTypes.shape({}).isRequired,
  numSelected: PropTypes.number.isRequired,
  handleBulkDelete: PropTypes.func.isRequired
};

export default withStyles(styles)(RegistrationCodeTableToolbar);
