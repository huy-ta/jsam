...
class StudentTable extends React.Component {
  ...
  render() {
    ...
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, session.attendance.students.length - page * rowsPerPage);
      ...
      <React.Fragment>
        <div className={classes.tableWrapper}>
          <Table className={classes.table}>
            ...
            <TableBody>
              {stableSort(session.attendance.students, getSorting(order, orderBy))
                .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                .map((row, index) => (
                  <StudentTableRow key={row.studentId} item={row} session={session} index={index} faults={faults} />
                ))}
              ...
              {emptyRows > 0 && session.attendance.students.length > 0 && (
                <TableRow style={{ height: 49 * emptyRows }}>
                  <TableCell colSpan={8} />
                </TableRow>
              )}
        ...
        <TablePagination
          ...
          component="div"
          count={session.attendance.students.length}
