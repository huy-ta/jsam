...
import { updateFaultOnServerById, deleteFaultOnServerById } from 'Services/faults/actions';
...
class FaultTableRow extends React.Component {
  state = {
    editModeOn: false,
    editSuccess: false,
    editLoading: false,
  ...
  handleConfirmEdit = async () => {
    ...
    const { _id } = input;
    const faultUpdateData = { ...input };

    this.setState(() => ({ editLoading: true }));
    try {
      const { dispatchUpdateFaultOnServerById } = this.props;
      await dispatchUpdateFaultOnServerById(_id, faultUpdateData);
  ...
  handleDelete = async () => {
    try {
      ...
      const { item } = this.props;
      const { dispatchDeleteFaultOnServerById } = this.props;
      await dispatchDeleteFaultOnServerById(item._id);
  ...
  render() {
    ...
    if (editModeOn) {
      ...
      if (editLoading) {
        editButtonTooltipTitle = 'Updating fault...';
      } else if (editPostLoading) {
        if (editSuccess) {
          editButtonTooltipTitle = 'Successfully updated fault';
        } else {
          editButtonTooltipTitle = 'Updating fault failed';
        }
      }
    }
    ...
    if (deleteLoading) {
      deleteButtonTooltipTitle = 'Deleting fault...';
    } else if (deletePostLoading) {
      if (deleteSuccess) {
        deleteButtonTooltipTitle = 'Successfully deleted fault';
      } else {
        deleteButtonTooltipTitle = 'Deleting fault failed';
      }
    }
...
FaultTableRow.propTypes = {
  classes: PropTypes.shape({}).isRequired,
  item: PropTypes.shape({}).isRequired,
  isSelected: PropTypes.bool.isRequired,
  handleClick: PropTypes.func.isRequired,
  dispatchUpdateFaultOnServerById: PropTypes.func.isRequired
};

const mapDispatchToProps = dispatch => ({
  dispatchUpdateFaultOnServerById: (_id, faultUpdateData) => dispatch(updateFaultOnServerById(_id, faultUpdateData)),
  dispatchDeleteFaultOnServerById: _id => dispatch(deleteFaultOnServerById(_id))
});

export default connect(
  undefined,
  mapDispatchToProps
)(withStyles(styles)(FaultTableRow));
